...
fn quilt_cabi_smoke_against_golden() {
    ...
    {
        let seal = take(unsafe { quilt_ledger_record(
            cell.as_ptr(),
    ...
    assert_eq!(unsafe { quilt_ledger_verify(cell.as_ptr()) }, 1, "chain must verify");
    ...
    assert_eq!(unsafe { quilt_ledger_verify(c("no.such.ledger").as_ptr()) }, -1);
    let bad =
    ...
    assert_eq!(unsafe { quilt_engine_load_sheet(null_mut(), std::ptr::null()) }, -1);
