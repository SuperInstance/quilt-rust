...
- **link_quality**: radio roads carry RSSI EWMA (driver-observed, dBm);
  ...
  Rung 1's "app: latency+loss bucket". `null` only before any observation.
...
cargo test -p quilt-wire
  12 unit  (frame golden/CRC, seq wrap/gap/restart, tlv strictness,
            walks chain + verify, peer stamping/gap/restart)
   ...
   1 pty loopback  — THE METAL PROOF: real kernel pty pair, firmware-twin
            sender (250 frames + injected line noise) → desktop peer;
            asserts 100% decode, seq 0..249 contiguous, walks/2 chain
            verifies, road stamped on every line, link_quality present.
```
...
- NOT proven here: anything on silicon. No ESP32 was attached; the sketch is
  ...
  RSSI stamping exists in the API but no radio driver feeds it yet.
