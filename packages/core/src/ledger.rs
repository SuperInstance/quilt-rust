...
impl LedgerEntry {
    ...
    fn seal(&self) -> Hash {
        let mut body =
            serde_json::to_value(self).expect("LedgerEntry is plain data; serialization is infallible");
...
impl CellLedger {
    ...
    pub fn settle_output_with(
        ...
        Ok(self.append_entry(pending.input, pending.ts, output.into(), ts, pending.provenance, expected))
    }
...
mod tests {
    ...
    fn open_inputs_do_not_balance() {
        ...
        let entry = ledger.settle_output(ticket, json!({"answer": 42}), 5_050).unwrap();
